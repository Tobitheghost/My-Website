...
def pt(p, n, t=titles):
    ...
    if print_log == True:
        print(t[n])
con = sqlite3.connect('tobitheghost/static/pages/parcel/Parcels_data.db')
cur = con.cursor()
cur.execute("""
...
def new_database():
    for x in range(len(data.index)):
        ...
        pt(percent,0)
        new__the_geom = [[x.split(" ")[1], x.split(" ")[0]] for x in item]
        pt(percent,3)
        ...
        if print_log == True:
            print(f"\033[0;32mMultipolygon:\033[0m {new__the_geom}")
        ...
        print(f"\033[0;32mNew Full Address:\033[0m {new_full_address}")
        try:
            cur.execute(f'INSERT OR IGNORE INTO Parcels VALUES(NULL,"{new__the_geom}",{new__KIVAPIN},"{new__APN}","{new__PLATNAME}","{new__LOT}","{new__BLOCK}","{new__TRACT}","{new__OWN_NAME}","{new__OWN_NAME2}","{owner_occupied}","{owner_occupied_str}","{new__OWN_ADDR}","{new__OWN_ADDR2}","{new__OWN_CITY}","{new__OWN_STATE}","{new__OWN_ZIP}","{new_OWNER_full_address}","{new__ADDRESS}","{new__ADDR}","{new__FRACTION}","{new__PREFIX}","{new__STREET}","{new__STREET_TYPE}","{new__SUITE}","{new_full_address}","{new__LANDUSECODE}","{new__LANDUSEDESC}","{new__ASSESSED_LAND_VALUE}","{new__ASSESSED_IMPROVE_VALUE}","{new__EXEMPT_LAND_VALUE}","{new__EXEMPT_IMPROVE_VALUE}","{new__ASSESSMENT_EFFECTIVE_DATE}","{new__LEGAL}","{new__SHAPE_AREA}","{new__SHAPE_LEN}")')
            con.commit()
        except sqlite3.OperationalError:
            errorlog.append(new_full_address)
    con.close()